...
class GeradorCrachasV3 extends Page implements HasForms
{
    use HasPageShield;
    ...
    protected static ?string $navigationLabel = 'Gerador de Crachás V3';

    protected static ?string $title = 'Gerador de Crachás V3';

    protected static ?string $slug = 'secretaria/gerador-crachas-v3';

    protected string $view = 'filament.pages.gerador-crachas-v3';
    ...
    public function form(Schema $schema): Schema
    {
        return $schema
            ->components([
                Section::make('Parâmetros de Geração (Versão 3 - Moveable)')
                    ->description('Selecione o modelo de crachá V3 e defina as pessoas para quem deseja gerar.')
                    ->schema([
                        Grid::make(2)
                            ->schema([
                                Select::make('template_id')
                                    ->label('Modelo de Crachá V3')
                                    ->options(TemplateCrachaV3::pluck('nome', 'id'))
                                    ->required()
                                    ->searchable()
                                    ->preload(),
    ...
    protected function getHeaderActions(): array
    {
        return [
            Action::make('ajuda')
                ...
                ->color('gray')
                ->modalHeading('Ajuda: Gerador de Crachás V3')
    ...
    {
        $html = '<p>Esta página permite realizar a geração de crachás em PDF utilizando os templates da **Versão 3 (Moveable)**.</p>';
        $html .= '<h3>Como usar?</h3>';
        $html .= '<ul>';
        $html .= '<li><strong>Modelo de Crachá V3:</strong> Selecione o layout V3 cadastrado no sistema.</li>';
    ...
    public function gerar(): ?StreamedResponse
    {
        ...
        $template = TemplateCrachaV3::find($templateId);

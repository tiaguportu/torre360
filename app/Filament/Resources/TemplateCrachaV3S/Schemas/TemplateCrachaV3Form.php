...
namespace App\Filament\Resources\TemplateCrachaV3S\Schemas;
...
class TemplateCrachaV3Form
{
    public static function configure(Schema $schema): Schema
    {
        return $schema
            ->components([
                Section::make('Configurações do Crachá V3 (Moveable)')
                    ->schema([
                        TextInput::make('nome')
                            ...
                            ->content(fn (?Model $record) => $record
                                ? new HtmlString('
                                    <div class="flex flex-col gap-2.5 items-start p-4 bg-slate-900 border border-slate-800 rounded-xl max-w-xl">
                                        <p class="text-sm font-semibold text-slate-200">Editor Canvas Interativo (Moveable)</p>
                                        <p class="text-xs text-slate-400">O layout deste template é editado com elementos HTML arrastáveis e redimensionáveis usando o Moveable. Ao clicar no botão abaixo, o editor será aberto em uma nova aba.</p>
                                        <a href="'.route('template-crachas-v3.editor', $record->id).'" target="_blank" class="inline-flex items-center gap-2 px-5 py-2.5 bg-violet-600 hover:bg-violet-500 text-white font-semibold rounded-lg text-xs transition-all active:scale-95 shadow-md shadow-violet-600/20">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                            Editar Canvas do Crachá V3
                                        </a>
                                    </div>
                                ')
                                : new HtmlString('

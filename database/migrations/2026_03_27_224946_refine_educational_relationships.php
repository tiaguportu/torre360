...
return new class extends Migration
{
    ...
    public function up(): void
    {
        if (config('database.default') === 'sqlite') {
            ...
            try {
                DB::statement('ALTER TABLE avaliacao ADD COLUMN data_prevista DATE');
            } catch (Exception $e) {
            }
            try {
                DB::statement('ALTER TABLE avaliacao ADD COLUMN nota_maxima DECIMAL(5,2)');
            } catch (Exception $e) {
            }
            try {
                DB::statement('ALTER TABLE avaliacao ADD COLUMN peso_etapa_avaliativa DECIMAL(5,2)');
            } catch (Exception $e) {
            }
        } else {

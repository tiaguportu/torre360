...
class QuestionarioRespostaFeedbackRecursivoTest extends TestCase
{
    use RefreshDatabase;

    public function test_pode_dar_feedback_vinculando_parent_id(): void
    {
        ...
        // 4. Testar acesso via requisição HTTP GET para garantir que a página de responder com parent_id carrega
        ...
        // 5. Testar a submissão do componente Livewire simulando o feedback
        ...
        // 6. Validar se o feedback foi gravado com o parent_id correto
        ...
        $feedbackResposta = QuestionarioResposta::where('parent_id', $parentResposta->id)->first();
        $this->assertNotNull($feedbackResposta);
        $this->assertEquals($parentResposta->id, $feedbackResposta->parent->id);
        $this->assertTrue($parentResposta->children->contains($feedbackResposta));

        // 8. Validar que o botão de comparação rápida (Comparar Feedbacks) está disponível na tela de detalhes
        Livewire::test(ViewQuestionarioResposta::class, [
            'record' => $parentResposta->id,
        ])
            ->assertActionExists('comparar_feedbacks');
    }
